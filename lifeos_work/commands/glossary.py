...
def command_glossary(args):
    _projects, _entries, data = read_current_data()
    terms = data["terms"]
    if args.kind:
...
def command_term_update(args):
    with transaction(args) as tx:
        if tx.idempotent_result():
            return
        entries = tx.data("entries")
        data = tx.data("glossary")
        events = tx.events
        term = find_item(data["terms"], args.id, "实体名词")
        previous_name = term.get("name")
        owners = {
        ...
        for value in args.alias:
            if value not in term.setdefault("aliases", []):
                term["aliases"].append(value)
        ...
        event = make_event(
            ...
            f"同步 {updated_task_refs} 条待办的负责人",
            sources=args.source,
        )
        tx.commit(("entries", "glossary") if updated_task_refs else "glossary", event)
