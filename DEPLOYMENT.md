...
SmartWork 与 cc-switch 分别使用 `scripts/sync-to-smartwork.sh` 和 `scripts/sync-to-ccswitch.sh`。写入前确认精确目标、授权和可恢复副本；同步后回读目标 `SKILL.md` 与直接引用。源码验证、CLI 安装和 Skill 同步不能互相证明。
