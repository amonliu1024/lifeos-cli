---
name: lifeos
description: 读取或更新 LifeOS 私有工作账本，数字化项目的 LifeOS 清单，生成、补写、重做或确认自然日日报。用于查询 LifeOS 中还有什么、最近要做什么、提醒、历史变化、成果复盘或实体关系；用于明确写入、更新、关闭、取消、归档或改期 Work 事实；用于在项目根创建或维护 lifeos-project.json；也用于回答今天或昨天做了什么，并把本机 Agent 会话、当日 Work 事件和本人补录整理为日报。
---

# LifeOS

通过唯一 Agent Skill 使用 LifeOS。先探测本机能力，再判断请求属于 Project、Work、Daily，还是 Daily 中经用户另行授权的 Work 写入，并只加载该分支需要的 reference。
